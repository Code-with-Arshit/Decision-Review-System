...
def play(speed):
    global flag
    print(f"you clicked on play next fast. speed is", speed)
    ...
    canvas.image = frame
    canvas.create_image(0, -100, image=frame, anchor=tkinter.NW)
    if flag:
...
window = tkinter.Tk()
window.title("DRS by Arshit the Programmer")
cv_img = cv2.cvtColor(cv2.imread("drs.jpg"), cv2.COLOR_BGR2RGB)
canvas = tkinter.Canvas(window, width=SET_WIDTH, height=SET_HEIGHT)
photo = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(cv_img))
img_on_canvas = canvas.create_image(210, 70, ancho=tkinter.NW, image=photo)
canvas.pack()
